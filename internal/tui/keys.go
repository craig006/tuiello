...
type KeyMap struct {
	...
	OpenCard, CopyCardURL                                 key.Binding
...
func NewKeyMap(cfg config.KeybindingConfig) KeyMap {
	return KeyMap{
		...
		OpenCard:         key.NewBinding(key.WithKeys(cfg.Board.OpenCard), key.WithHelp(cfg.Board.OpenCard, "open card")),
		CopyCardURL:      key.NewBinding(key.WithKeys(cfg.Board.CopyCardURL), key.WithHelp(cfg.Board.CopyCardURL, "copy card url")),
